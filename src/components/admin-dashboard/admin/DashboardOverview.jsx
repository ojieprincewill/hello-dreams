  ...
  return (
    <div className="space-y-6 xl:space-y-8">
              ...
              <CardContent className="p-4 xl:p-6">
                <div className="flex items-start justify-between">
                  <div className="flex-1 min-w-0">
                    <p className="text-sm xl:text-base font-medium text-gray-600 mb-2">
                      {stat.title}
                    </p>
                    ...
                    {/* Monthly Value */}
                    <div className="mb-3">
                      <p className="text-sm xl:text-base font-semibold text-gray-700">
                        {stat.monthlyValue}
                      </p>
                    ...
                    <div className="flex items-center">
                      {parseFloat(stat.change) >= 0 ? (
                        <TrendingUp
                          size={14}
                          className="text-green-500 mr-1 xl:w-4 xl:h-4"
                        />
                      ) : (
                        <TrendingUp
                          size={14}
                          className="rotate-180 text-red-500 mr-1 xl:w-4 xl:h-4"
                        />
                      )}
                      <span
                  ...
                  <div
                    className={`p-2 xl:p-3 rounded-full ${stat.bgColor} flex-shrink-0 ml-3`}
                  >
                    <Icon size={20} className={`${stat.color} xl:w-6 xl:h-6`} />
                  </div>
                </div>
              </CardContent>
            </Card>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 xl:gap-6">
            ...
            {loading ? (
              ...
              <div className="space-y-3 xl:space-y-4">
                {activity.data?.slice(0, 3).map((item, index) => (
                  <div
                    key={index}
                    className={`flex items-center space-x-3 p-2 xl:p-3 rounded-lg ${
        ...
        <Card>
          <CardHeader className="pb-3 xl:pb-6">
            <CardTitle className="text-lg xl:text-xl">Quick Actions</CardTitle>
          </CardHeader>
          <CardContent>
              ...
              {quickActions.map((action, index) => {
                ...
                return (
                  <button
                    ...
                    className={`p-3 xl:p-4 ${action.bgColor} ${action.hoverColor} rounded-lg transition-colors text-left cursor-pointer`}
                  >
                    <Icon
                      className={`${action.color} mb-2 xl:w-6 xl:h-6`}
                      size={20}
                    />
                    <p
                      className={`font-medium text-sm xl:text-base ${action.color
                        .replace("text-", "text-")
                        .replace("-600", "-900")}`}
                    >
                      {action.title}
                    </p>
                    <p className={`text-xs xl:text-sm ${action.color}`}>
                      {action.description}
                    </p>
                  </button>
                );
